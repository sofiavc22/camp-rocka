...
export default function Booking(){
  ...
  async function checkout(event:React.FormEvent){
    ...
    if(!date||!endDate||nights<1)return;
    setLoading(true);setError("");
    try{
      const response=await fetch("/api/crear-checkout",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({startDate:date,endDate})});
      const data=await response.json();
      if(!response.ok||!data.url)throw new Error(data.error||"No pudimos iniciar el pago.");
      window.location.assign(data.url);
    }catch(err){setError(err instanceof Error?err.message:"No pudimos iniciar el pago. Inténtalo nuevamente.");setLoading(false)}
  }

  return <form className="bookingCard" onSubmit={checkout}>
    ...
    {error&&<p role="alert" style={{color:"#9f2d20",fontWeight:700}}>{error}</p>}
    <button className="primary bookingButton" disabled={!date||!endDate||loading}>{loading?"Abriendo pago seguro…":date&&endDate?`Continuar al pago · ${money(total)}`:"Elige las fechas para continuar"}</button>
